...
5. **Visible when interactive:** a system tray icon so the user can tell the app is running (console launch and/or interactive session path).
...
- No polished GUI beyond tray presence; no MSI/store packaging required for v1 (scripts / `sc.exe` / simple install helper is enough).
- No multi-TV management.
- Interactive console mode does not need to become a full WinForms app yet — dual-mode (service vs console) is enough for interim use.
